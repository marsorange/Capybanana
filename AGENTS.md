...
- **账号 / Agent 连接（云存档）**：手机号免密登录（`POST /api/auth/login`，手机号即身份，无验证码）→ 服务器为新账号建一只宠物（网页用 `randomCompanion()` 随机生成后 `POST /api/agent/create`）→ 拿到绑定链接 `Read <域名>/agent/skill.md?bind=<token>`。AI Agent 读 `skill.md`（`src/app/agent/skill.md/route.ts`，按 token 动态生成带真实接口的说明）后，用同一套 **`/api/agent/*`**（`pet/create/pack/travel/battle/stay/pat/say/diary/collect/postcards/feed`，bind 令牌鉴权：`?bind=` 或 `Authorization: Bearer`）操作**同一只**云宠物。服务端复用纯逻辑：`src/server/engine.ts` 的 `tickSave` 包 `advanceLifecycle`，outcome 合并抽到共享的 `src/game/applyOutcome.ts`（`gameStore.tick` 也用它）；存储在 `src/lib/kv.ts`（Upstash/Vercel KV，未配置则进程内存回退）+ `src/server/store.ts`。登录后网页变“瘦客户端”：`gameStore` 的 `cloud` 切片把 `prepareBag/collectPostcard/createCompanion` 路由到服务器，`GameRoot` 改用 `cloudPull` 每 5s 拉取、本地不再随机结算。
- **Agent 驱动的玩法（核心）**：云宠物**不再自己出门**——主人在网页备好「今日包裹」（物品 + 心愿），宠物停在 `ready` 待命，由 **Agent** 看状态 + 包裹（`summarizePet` 现含 `bag`/`choices`）后替它决定今天怎么过：`startTravel`（旅行，可指定 `destination`，归来寄明信片）/ `startBattle`（找 Claw 较量，`resolveDay` 的 `resolveClaw` 按勇敢+体力+`protective`物+羁绊+运气出胜/负/平+战利品）/ `stayHome`（在家/院子/养伤，立即结算）。机制：`Trip.intent`（`OutcomeKind|"auto"|"quiet"`）决定 `resolveDay` 走哪种收场；`engine.packBag` 用 `clock.ts` 的 `NO_AUTO_DEPART` 哨兵把 `departAt` 设成永不触发，所以 `advanceLifecycle` 的 ready→traveling 自动出门只在**本地访客模式**（`gameStore.prepareBag` 仍 `scheduleDeparture`）触发，云端纯靠 Agent。**日记**：Agent 每天 `POST /api/agent/diary`（≤200 字，以宠物口吻）写一篇，`engine.writeDiary` 按 UTC 日期 upsert（同日覆盖、留近 90 篇）存 `CloudSave.diary`，`summarizePet` 暴露 `latestDiary`/`wroteDiaryToday`，`GET /api/agent/diary?limit=` 回看。**已知缺口**：`secretProgress`（多日秘密铺垫）与明信片生图持久化仍是客户端本地的，云宠物暂不享有，后续可下沉到服务端。
